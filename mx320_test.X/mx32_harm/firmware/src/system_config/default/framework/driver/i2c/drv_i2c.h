...
/* I2C Slave Device Address
  ...
  Summary:
    Data type for a slave device address.  Note, value should be left shifted so that address bits 
    A6 through A0 are located in bits A7 through A1 of any variable of this type.
...
/* I2C Buffer Event Callback
  ...
  Summary:
    Points to a callback which allows the application to respond to reads or 
    writes based on the R/W bit received by the I2C slave.
  ...
  Description:
    ...
    mechanism, the driver client can initiate an operation. The Slave can 
    issue a READ or WRITE based on the status of R/W bit received from the 
    Master

  Parameters:
   DRV_I2C_BUFFER_EVENT - This should be either DRV_I2C_BUFFER_SLAVE_READ_REQUESTED
   or DRV_I2C_BUFFER_SLAVE_WRITE_REQUESTED. The callback function should either 
   place a receive or transmit transaction into the driver's queue based on this value.
